...
void write_png(const char* filename, png_bytep* rows, int width, int height) {
  ...
  for (int i = 0; i < height; ++i) {
    png_write_row(png_ptr, rows[height - i - 1]);
  }
...
int main(int argc, char** argv) {
  ...
  png_bytep* all_rows;
	...
	if (rank == 0) {
		// draw and cleanup
		write_png(filename, all_rows, width, height);
		free(_all_rows);
	}
