...
__global__ void blockedFloydWarshallPhase1(int n, int blockId, int* d, size_t pitch) {
	...
	int idxIJ = (y + blockId * BLOCK_SIZE) * pitch + (x + blockId * BLOCK_SIZE);
...
__global__ void blockedFloydWarshallPhase2(int n, int blockId, int* d, size_t pitch) {
	...
	if (blockIdx.y == 0) {
		...
		idxIJ = (y + blockId * BLOCK_SIZE) * pitch + (x + blockIdx.x * BLOCK_SIZE);
	} else {
		// i = y + blockIdx.x * BLOCK_SIZE;
		idxIJ = (y + blockIdx.x * BLOCK_SIZE) * pitch + (x + blockId * BLOCK_SIZE);
	}
	// idxIJ = i * pitch + j;
...
__global__ void blockedFloydWarshallPhase3(int n, int blockId, int* d, size_t pitch) {
	...
	int idxIJ = (y + blockIdx.y * BLOCK_SIZE) * pitch + (x + blockIdx.x * BLOCK_SIZE);
	...
	cacheBaseColD[y][x] = d[(y + blockIdx.y * BLOCK_SIZE) * pitch + (x + blockId * BLOCK_SIZE)];
	...
	cacheBaseRowD[y][x] = d[(y + blockId * BLOCK_SIZE) * pitch + (x + blockIdx.x * BLOCK_SIZE)];
	__syncthreads();
