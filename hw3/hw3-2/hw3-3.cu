...
__global__ void blockedFloydWarshallPhase3(int n, int blockId, int* d, size_t pitch, int yOffset) {
	// skipping the base blocks (from phase 1, 2)
	if (blockIdx.x == blockId || blockIdx.y + yOffset == blockId) return;
	...
	i = y + (blockIdx.y + yOffset) * BLOCK_SIZE;
	j = x + blockId * BLOCK_SIZE;
...
int main(int argc, char** argv) {
	...
	#pragma omp parallel num_threads(2)
	{
		...
		cudaMalloc((void**)&deviceD[threadId], n * n * sizeof(int));
		...
		cudaMemcpy(deviceD[threadId] + yOffset * BLOCK_SIZE * n, hostD + yOffset * BLOCK_SIZE * n, gridPhase3.y * BLOCK_SIZE * n * sizeof(int), cudaMemcpyHostToDevice);

		for (int blockId = 0; blockId < numberOfBlocks; ++blockId) {
			if (blockId >= yOffset && blockId < yOffset + gridPhase3.y) {
				cudaMemcpy(hostD + blockId * BLOCK_SIZE * n, deviceD[threadId] + blockId * BLOCK_SIZE * n, BLOCK_SIZE * n * sizeof(int), cudaMemcpyDeviceToHost);
				// cudaMemcpy(deviceD[threadId ^ 1] + blockId * BLOCK_SIZE * n, deviceD[threadId] + blockId * BLOCK_SIZE * n, BLOCK_SIZE * n * sizeof(int), cudaMemcpyDeviceToDevice);
			}
			#pragma omp barrier

			if (blockId < yOffset || blockId >= yOffset + gridPhase3.y) {
				cudaMemcpy(deviceD[threadId] + blockId * BLOCK_SIZE * n, hostD + blockId * BLOCK_SIZE * n, BLOCK_SIZE * n * sizeof(int), cudaMemcpyHostToDevice);
			}

			blockedFloydWarshallPhase1<<<gridPhase1, threadsPerBlock>>>(n, blockId, deviceD[threadId], n);
			blockedFloydWarshallPhase2<<<gridPhase2, threadsPerBlock>>>(n, blockId, deviceD[threadId], n);
			blockedFloydWarshallPhase3<<<gridPhase3, threadsPerBlock>>>(n, blockId, deviceD[threadId], n, yOffset);
		}

		cudaMemcpy(hostD + yOffset * BLOCK_SIZE * n, deviceD[threadId] + yOffset * BLOCK_SIZE * n, gridPhase3.y * BLOCK_SIZE * n * sizeof(int), cudaMemcpyDeviceToHost);
	}
	...
	/* cudaMemcpy2D */
	// cudaMemcpy2D(hostD, n * sizeof(int), deviceD[0], pitch, n * sizeof(int), n, cudaMemcpyDeviceToHost);
	// cudaFree(deviceD);
